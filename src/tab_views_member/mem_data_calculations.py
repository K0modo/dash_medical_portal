...
class MemberCalculations:
    ...
    def annual_average_charge(self):
        return self.mem_table.charge_allowed.mean()
    ...
    def claims_by_period(self):
        group_data = self.mem_table.groupby('period', as_index=False)['claim_id'].count().sort_values(
            'period')
        return group_data
...
class GridStats:
    ...
    def get_member_claim_detail(self):
        group_data = (self.table.loc[:, ['claim_id', 'period', 'injury_disease', 'specialty', 'facility_class',
                                         'charge_allowed', 'deduct_copay']])
